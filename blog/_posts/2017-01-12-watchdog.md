...
```
*/1 * * * * /root/mesh-watchdog.sh
```

It can run once a minute as it detects whether a reset has just occurred and will wait 20 minutes before resetting again. I added the 20 minute delay so the router is still functional without an internet gateway.
