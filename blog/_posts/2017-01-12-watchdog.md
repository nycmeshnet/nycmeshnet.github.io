...
It can run once a minute as it detects whether a network restart has just occurred and will wait 20 minutes before restarting again. I added the 20 minute delay so the router is still functional without an internet gateway. In the new version 1.2.0, I've added a lock so there can't be two instances running at once, and added a ```sleep 60``` in the ping so it won't constantly reset with a shaky gateway.
